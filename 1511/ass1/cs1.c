...
int main(void) {
    ...
    while (scanf(" %c", &command) != EOF) { 
        ...
        if (command == 'n') { 
            ...
            int left_up = section_points(left_up_row, left_up_col, top_left_row, 
            top_left_col, hint_size, map); 

            int right_up_row = player_row - 1; 
            int right_up_col = player_col + 1; 
            int top_right_row = right_up_row + hint_size; 
            int top_right_col = right_up_col + hint_size; 
            int right_up = section_points(right_up_row, right_up_col, top_right_row, 
            top_right_col, hint_size, map);

            int left_down_row = player_row + 1; 
            int left_down_col = player_col - 1; 
            int down_left_row = left_down_row + hint_size; 
            int down_left_col = left_down_col + hint_size; 
            int left_down = section_points(left_down_row, left_down_col, down_left_row, 
            down_left_col, hint_size, map);

            int right_down_row = player_row + 1; 
            int right_down_col = player_col + 1; 
            int down_right_row = right_down_row + hint_size; 
            int down_right_col = right_down_col + hint_size; 
            int right_down = section_points(right_down_row, right_down_col, down_right_row, 
            down_right_col, hint_size, map);

            if (left_down == left_up) && (right_up == right_down) && 
            (right_down == left_up) { 
                printf("Left Up"); 
            }
            if ((left_up < left_down) || (left_up < right_down) ||
            (left_up < right_up)) { 
                printf("Left Up"); 
            }
            if ((left_down < left_up) || (left_down < right_down) ||
            (left_down < right_up)) { 
                printf("Left Down"); 
            }
            if ((right_up < left_down) || (right_up < right_down) ||
            (right_up < left_up)) { 
                printf("Right Up"); 
            }
            if ((right_down < left_down) || (right_down < right_up) ||
            (right_down < right_up)) { 
                printf("Right Down"); 
            }
        }
...
int new_col, int hint_size, struct location map[SIZE][SIZE]) 
{
    ...
    int denominator = hint_size - boulder_count; 
